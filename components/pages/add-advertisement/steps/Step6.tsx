...
const Step6 = forwardRef<Step6Handle, Step6Props>(
  ({ isSubmitting, advertisementFormState, onStepCompleted, onValidationStatusChanged }, ref) => {
    ...
    const saveStep = async (): Promise<{ advertisementData: AdvertisementLocation }> => {
      ...
      try {
        const response = await advertisementService.updateAdvertisementLocation({
          AdUnitUpdate: {
            Id: advertisementId,
            Step: 6,
        ...
        if (response.IsSuccess) {
          showToast('تم حفظ موقع الوحده بنجاح', 'success');
          const locationData = {
            ...locationForm,
            mapLocation: {
          ...
          showToast(response.Error || 'فشل في حفظ موقع الوحده', 'error');
          throw new Error(response.Error || 'فشل في حفظ موقع الوحده');
        }
      } catch (error: any) {
        showToast(error.message || 'فشل في حفظ موقع الوحده', 'error');
        throw error;
      }
    };
    ...
    return (
      <div className={styles.stepContent}>
        <h3 className="mb-4">موقع الوحدة</h3>
