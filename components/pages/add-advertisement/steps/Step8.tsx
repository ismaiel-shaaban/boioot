...
const Step8 = forwardRef<Step8Handle, Step8Props>(
  ({ isSubmitting, advertisementFormState, onStepCompleted, onValidationStatusChanged }, ref) => {
    ...
    const { advertisementId, clearAdvertisementId } = useAdvertisementState();
    ...
    const saveStep = async (): Promise<{ advertisementData: any }> => {
      ...
      if (!advertisementId) {
        throw new Error('Unit ID not available');
      }
      ...
      try {
        ...
        const response = await advertisementService.updateAdvertisementFeatures({ AdUnitUpdate: featuresData });

        if (response.IsSuccess) {
          showToast('تم حفظ مميزات الوحدة بنجاح', 'success');
          onStepCompleted({ advertisementData: advertisementFormState });

          setTimeout(() => {
            clearAdvertisementId();
            if (typeof window !== 'undefined') {
              localStorage.removeItem('advertisementFormState');
            }
            router.push('/home');
          ...
          showToast(response.Error || 'فشل في حفظ مميزات الوحدة', 'error');
          throw new Error(response.Error || 'فشل في حفظ مميزات الوحدة');
        }
      } catch (error: any) {
        showToast(error.message || 'فشل في حفظ مميزات الوحدة', 'error');
        throw error;
      }
    };
    ...
    return (
      <div className={styles.stepContent}>
        <div className="d-flex justify-content-between align-items-center mb-3">
          <p className="mb-0">اختر المميزات المتوفرة في الوحدة</p>
