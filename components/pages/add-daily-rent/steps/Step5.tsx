...
const Step5 = forwardRef<Step5Handle, Step5Props>(
  ({ isSubmitting, advertisementFormState, onStepCompleted, onValidationStatusChanged }, ref) => {
    ...
    const uploadFile = async (file: File): Promise<string> => {
      if (!advertisementId) {
        throw new Error('معرف الإعلان اليومي غير متوفر');
      }
    ...
    const saveStep = async (): Promise<{ advertisementData: AdvertisementMedia }> => {
      ...
      try {
        ...
        const response = await dailyRentService.uploadAdvertisementMedia({
          Unit: {
            Id: advertisementId,
            Step: 5,
        ...
        if (response.IsSuccess) {
          ...
          onStepCompleted({ advertisementData: finalForm });
          showToast('تم حفظ وسائط الإعلان اليومي بنجاح', 'success');
          setIsSavingStep(false);
          return { advertisementData: finalForm };
        } else {
          showToast(response.Error || 'فشل في حفظ وسائط الإعلان اليومي', 'error');
          setIsSavingStep(false);
          throw new Error(response.Error || 'فشل في حفظ وسائط الإعلان اليومي');
        }
      } catch (error: any) {
        ...
        {isSavingStep && (
          <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style={{ backgroundColor: 'rgba(255,255,255,0.9)', zIndex: 9999 }}>
            <div className="text-center">
              ...
              <p className="mt-2">جاري حفظ وسائط الإعلان اليومي...</p>
            </div>
          </div>
        )}

        <h4 className="mb-3">صور وفيديوهات الإعلان اليومي</h4>
