...
const Step6 = forwardRef<Step6Handle, Step6Props>(
  ({ isSubmitting, advertisementFormState, onStepCompleted, onValidationStatusChanged }, ref) => {
    ...
    const initializeMap = () => {
      ...
      let initialLat = 33.5138;
      let initialLng = 36.2765;
    ...
    const saveStep = async (): Promise<{ advertisementData: AdvertisementLocation }> => {
      ...
      try {
        const response = await dailyRentService.updateAdvertisementLocation({
          Unit: {
            Id: advertisementId,
            Step: 6,
        ...
        if (response.IsSuccess) {
          showToast('تم حفظ موقع الإعلان اليومي بنجاح', 'success');
          const locationData = {
            ...locationForm,
            mapLocation: {
          ...
          showToast(response.Error || 'فشل في حفظ موقع الإعلان اليومي', 'error');
          throw new Error(response.Error || 'فشل في حفظ موقع الإعلان اليومي');
        }
      } catch (error: any) {
        showToast(error.message || 'فشل في حفظ موقع الإعلان اليومي', 'error');
        throw error;
      }
    };
    ...
    return (
      <div className={styles.stepContent}>
        <h3 className="mb-4">موقع الإعلان اليومي</h3>
