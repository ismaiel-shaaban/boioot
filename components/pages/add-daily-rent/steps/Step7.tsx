  ...
  ({ isSubmitting, advertisementFormState, onStepCompleted, onValidationStatusChanged }, ref) => {
    ...
    const saveStep = async (): Promise<{ advertisementData: AdvertisementSpecifications }> => {
      ...
      try {
        ...
        const response = await dailyRentService.updateUnitInfoDetails({ Unit: unitData });

        if (response.IsSuccess) {
          showToast('تم حفظ تفاصيل الإعلان اليومي بنجاح', 'success');
          ...
          showToast(response.Error || 'فشل في حفظ تفاصيل الإعلان اليومي', 'error');
          throw new Error(response.Error || 'فشل في حفظ تفاصيل الإعلان اليومي');
        }
      } catch (error: any) {
        showToast(error.message || 'فشل في حفظ تفاصيل الإعلان اليومي', 'error');
        throw error;
      }
    };
    ...
    return (
      <div className={styles.stepContent}>
        <h3 className="mb-4">مواصفات الإعلان اليومي</h3>
