...
import styles from './BlogsCategoriesClient.module.css';
...
export default function BlogsCategoriesClient() {
  ...
  const loadCategories = async () => {
    ...
    try {
      const response = await communityService.getCategoriesWithLatestPosts("1");
      if (response?.IsSuccess) {
        ...
        showToast(response?.Message || 'فشل في تحميل المدونات', 'error');
      }
    } catch (error: any) {
      const errorMessage = error?.message || 'حدث خطأ أثناء جلب المدونات';
  ...
  return (
    <div className={styles.communitiesMain}>
      <div className="container">
        {/* Communities Filter */}
        <div className={`${styles.communitiesFilter} d-flex justify-content-between align-items-center mb-3`} role="region" aria-label="تصفية المدونات">
          <div className={styles.filterItem}>كل المدونات</div>
        </div>
        
        {/* Communities Grid */}
        <div className={styles.communitiesGrid} role="region" aria-label="قائمة المدونات">
          {loading ? (
            <div className="text-center py-5" aria-live="polite">
              ...
              <p className="mt-3">جاري تحميل المدونات...</p>
            </div>
          ) : error ? (
            ...
            <div className="row">
              {communities.map((community) => (
                <div key={community.Id} className="col-md-6 col-lg-4 mb-4 ">
                  <div className={styles.communityCard} role="article" aria-label="بطاقة المدونة">
                    <div className={styles.communityHeader}>
                      <div className={styles.communityIcon}>
                        <Link href={`/list-blogs/${community.Id}`}>
                          <img
                      ...
                      <h3 className={styles.communityTitle}>
                        <Link href={`/list-blogs/${community.Id}`} aria-label="اسم المدونة">
                          {community.Name}
                        </Link>
                      </h3>
                      <div className={styles.communityStats} aria-label="إحصائيات المدونة">
                        <span>
                      ...
                      <div className={styles.communityLatest} aria-label="أحدث مشاركة">
                        <div className={styles.latestPost}>
                          ...
                          <div className={styles.postTitle}>
                            <Link
                              href={`/post-details/1/${community.Id}/${community.LatestPost.Id}`}
            ...
            <div className="row">
              <div className="col-12">
                <div className="alert alert-info text-center" role="alert" aria-label="لا يوجد مدونات">
                  لايوجد مجتمعات
                </div>
              </div>
            </div>
