...
const Step8 = forwardRef<Step8Handle, Step8Props>(
  ({ isSubmitting, advertisementFormState, onStepCompleted, onValidationStatusChanged }, ref) => {
    ...
    const { specialOrderId: advertisementId, clearSpecialOrderId } = useSpecialOrderState();
    ...
    const saveStep = async (): Promise<{ advertisementData: any }> => {
      ...
      try {
        ...
        const response = await specialOrderService.updateAdvertisementFeatures({ Order: featuresData });

        if (response.IsSuccess) {
          showToast('تم حفظ مميزات الطلب الخاص بنجاح', 'success');
          onStepCompleted({ advertisementData: advertisementFormState });

          setTimeout(() => {
            clearSpecialOrderId();
            if (typeof window !== 'undefined') {
              localStorage.removeItem('specialOrderFormState');
          ...
          showToast(response.Error || 'فشل في حفظ مميزات الطلب الخاص', 'error');
          throw new Error(response.Error || 'فشل في حفظ مميزات الطلب الخاص');
        }
      } catch (error: any) {
        showToast(error.message || 'فشل في حفظ مميزات الطلب الخاص', 'error');
        throw error;
      }
    };
    ...
    return (
      <div className={styles.stepContent}>
        <div className="d-flex justify-content-between align-items-center mb-3">
          <p className="mb-0">اختر المميزات المتوفرة في الطلب الخاص</p>
